# Extracted from GoogleCloudPlatform/agent-starter-pack/src/base_template/deployment/terraform/
# Part of the Universal ADK Agent Starter Kit
# Original source: https://github.com/GoogleCloudPlatform/agent-starter-pack/src/base_template/deployment/terraform/
...
resource "google_bigquery_dataset" "feedback_dataset" {
  for_each      = local.deploy_project_ids
  project       = each.value
  dataset_id    = replace("{{project_name}}_feedback", "-", "_")
  friendly_name = "{{project_name}}_feedback"
  location      = var.region
...
resource "google_bigquery_dataset" "telemetry_logs_dataset" {
  for_each      = local.deploy_project_ids
  project       = each.value
  dataset_id    = replace("{{project_name}}_telemetry", "-", "_")
  friendly_name = "{{project_name}}_telemetry"
  location      = var.region
  depends_on    = [resource.google_project_service.cicd_services, resource.google_project_service.shared_services]
}
...
module "log_export_to_bigquery" {
  ...
  log_sink_name          = "{{project_name}}_telemetry"
...
module "feedback_export_to_bigquery" {
  ...
  source                = "terraform-google-modules/log-export/google"
  version               = "10.0.0"
  log_sink_name         = "{{project_name}}_feedback"
  parent_resource_type  = "project"
  parent_resource_id    = each.value
